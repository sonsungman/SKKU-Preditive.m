...
<Table 3>은 실적 데이터를 활용하여 피보험자들의 과거 보험사고 이력이 포아송 분포를 따른다고 가정하여 GLMM 분석을 한 결과이다. <Table 3>을 보면 피보험자의 비관찰 정보에 대한 위험은 θ~ lognormal(0,〖1.223〗^2)을 따르는 것을 알 수 있다. 

<Table 3> is the result of GLMM analysis assuming that the insured's past insurance accident history follows the Poisson distribution using data. Looking at <Table 3>, it can be seen that the risk of non-observation information of the insured follows θ~ lognormal(0,〖1.223〗^2).
